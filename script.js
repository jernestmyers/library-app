...
function isBookDataValid() {
  ...
  userInputFields.forEach((input) => {
    if (!input.validity.valid) {
      isValid = false;
      if (input.id === `page-length`) {
        togglePageLengthError(isValid);
      }
      return isValid;
    }
  });
...
function togglePageLengthError(boolean) {
  const errorMessage = document.querySelector(`#page-input-error`);
  if (!boolean) {
    errorMessage.textContent = `*Enter a number between 1 and 2500.`;
  } else {
